...
import com.valleapp.comandas.IUtil.INota;
import com.valleapp.comandas.IUtil.ITeclados;
import com.valleapp.comandas.Util.AdaptadorComanda;
import com.valleapp.comandas.Util.AdaptadorPedidos;
import com.valleapp.comandas.Util.LaComanda;
import com.valleapp.comandas.Util.Nota;
import com.valleapp.comandas.Util.ServicioCom;
import com.valleapp.comandas.Util.Teclados;
...
public class Comanda extends FragmentActivity implements  INota, IComanda, ITeclados {
    ...
    private void cargarPreferencias() {
        ...
        try {
            ...
            if(!pref.isNull("sec")) {
                sec = dbSecciones.find(pref.getString("sec"));
            }else{
            ...
            if (lsart.length() > 0) {
                ...
                for (int i = 0; i < lsart.length(); i++) {
                    ...
                    if(sec.getBoolean("Es_promocion")){
                         Boolean aplicable = sec.getBoolean("Es_aplicable");
                         if(!aplicable) btn.setBackgroundColor(Color.RED);
                         else{
                             Double precio = m.getDouble("Precio");
                             Double descuento = sec.getDouble("Descuento");
                             m.put("Precio",precio*descuento);
                         }
                    }
    ...
    public void clickMenu(View v) throws JSONException {
        sec = dbSecciones.find(v.getTag().toString());
        RellenarBotonera();
    }
    ...
    public void clickEnviarComanda(View v){
        try{
          ...
          p.add(new BasicNameValuePair("mensaje", comanda.getMensaje()));
          if(myServicio!=null){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        teclados = new Teclados(this, dbSecciones.getAll());
    ...
    public void cobrarExtra(View view){
        try {
            boolean aplicable = sec.getBoolean("Es_aplicable");
            if(aplicable)  aplicable = false;
            else aplicable = true;
            sec.put("Es_aplicable", aplicable);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        dbSecciones.updateRow(sec);
        RellenarBotonera();
    }
}