...
public class DbTeclas extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS  teclas (ID INTEGER PRIMARY KEY, Nombre TEXT, P1 DOUBLE, P2 DOUBLE, Precio DOUBLE," +
                         " RGB TEXT, IDSeccion INTEGER,  Orden INTEGER, Nombre_sec TEXT)");
    }
       ...
       // Insert the new row, returning the primary key value of the new row
        for (int i= 0 ; i<datos.length();i++){
            ...
            try {
                 ...
                 values.put("Nombre_sec", datos.getJSONObject(i).getString("Nombre_sec"));
                 values.put("Orden", datos.getJSONObject(i).getString("OrdCom"));
    ...
    public JSONArray getAll(String id, int tarifa)
    {
        ...
        Cursor res =  db.rawQuery( "SELECT * FROM teclas WHERE Nombre_sec='"+id +"' ORDER BY Orden DESC", null );
        res.moveToFirst();
