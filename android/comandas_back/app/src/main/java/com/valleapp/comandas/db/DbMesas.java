...
public class DbMesas extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS mesas (ID INTEGER PRIMARY KEY, Nombre TEXT, RGB TEXT, abierta TEXT, Tarifa INT, IDZona INTEGER, num INTEGER, Orden INTEGER)");
    }
    ...
    public void RellenarTabla(JSONArray datos){
        ...
        for (int i= 0 ; i<datos.length();i++){
            ...
            try {
                ...
                values.put("Tarifa", datos.getJSONObject(i).getInt("Tarifa"));
    ...
    public JSONArray getAll(String id)
    {
        ...
        while(res.isAfterLast() == false){
            try{
                ...
                obj.put("Tarifa", res.getString(res.getColumnIndex("Tarifa")));
                lista.put(obj);
            } catch (JSONException e) {
                e.printStackTrace();
            }
    ...
    public void updateByZona(JSONArray datos, String idz) {
        SQLiteDatabase db = this.getWritableDatabase();
        try {
            db.execSQL("UPDATE mesas SET abierta='false', num=0 WHERE IDZona="+idz);
            for (int i = 0; i < datos.length(); i++) {
                db.execSQL("UPDATE mesas SET abierta='true', num="
                        + datos.getJSONObject(i).getString("num") + " WHERE ID=" + datos.getJSONObject(i).getString("IDMesa"));
            }
